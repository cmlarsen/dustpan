...
- **Git worktrees** in every repo under your roots: dirty, unpushed, merged (via `git merge-base`, or a `gh pr list` PR whose head commit is this worktree's HEAD; fork PRs and branches other PRs target are ignored), idle, or has a process running in it. Removal goes through `git worktree remove`, so git refuses to remove a checkout with changes. A worktree whose folder is missing is SAFE only when git confirms it's gone; one on an unmounted `/Volumes` drive, or one that can't be checked, is REVIEW.
- **Xcode DerivedData**: reads each folder's `info.plist` to see whether the workspace that built it still exists.
- **Simulators**: last use (`device.plist`), size of the largest app data, and `(wt-name)` / `(wt-name--variant)` clones whose worktree is gone. A `wt-` clone that matches no worktree Dustpan knows about is REVIEW. It runs `simctl erase` or `simctl delete`.
...
- **Package caches**: pnpm, uv, Homebrew, CocoaPods, npm, bun, cargo, Playwright, Android emulators and system images, Hugging Face, and others. Prune commands (`pnpm store prune`, `uv cache prune`) are SAFE. `brew cleanup --prune=all` is REVIEW because it also removes old formula versions. A cache that gets deleted outright is REVIEW while you've used it within `stale_days`, because deleting it only buys a re-download.
...
- When Dustpan can't determine a fact (a git, simctl, or lsof call fails or times out, or a path can't be stat'ed), the item is REVIEW, never SAFE.
